...
static void clcd_write(unsigned char byte, unsigned char mode) {
    CLCD_RS = mode; //RE2-> RS
    CLCD_DATA_PORT = byte;  //PORTD

    CLCD_EN = HI;           //Enable high
    __delay_us(100);
    CLCD_EN = LOW;          //Enable low
...
static void init_display_controller(void) {
    ...
    clcd_write(EIGHT_BIT_MODE, INST_MODE); //0x33,0  -> data, RS
    __delay_us(4100);
    ...
    __delay_us(500);
    clcd_write(DISP_ON_AND_CURSOR_OFF, INST_MODE); //0x0C
    __delay_us(100);
}

void init_clcd(void) {
    /* Setting the CLCD Data Port as Output */
    CLCD_DATA_PORT_DDR = 0x00; //TRISD
...
void clcd_putch(const char data, unsigned char addr) {
    clcd_write(addr, INST_MODE);    //0
    clcd_write(data, DATA_MODE);    //1
}
